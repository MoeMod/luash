require("common/mathlib")
require("common/const")
local CGALIL = class("CGALIL", CBasePlayerWeapon)
...
local WeaponConfig = {
    ...
    Weight = 25,
    V_Model = "models/v_galil.mdl",
    P_Model = "models/p_galil.mdl",
    W_Model = "models/w_galil.mdl",
    EventFile = "addons/luash/events/event_galil.lua",
    AnimExtension = "rifle",
...
LINK_ENTITY_TO_CLASS(WeaponConfig.ClassName, CGALIL)

function CGALIL:Spawn() -- override
...
function CGALIL:Precache() -- override
	...
	self.m_usFireGALIL = PRECACHE_EVENT(1, WeaponConfig.EventFile);
end

function CGALIL:GetItemInfo(p) -- override
...
function CGALIL:PrimaryAttack() -- override
...
function CGALIL:WeaponFire(flSpread, flCycleTime, fUseAutoAim)
	...
	PLAYBACK_EVENT_FULL(flags, self.m_pPlayer, self.m_usFireGALIL, 0, {0,0,0}, {0,0,0}, vecDir[1], vecDir[2], self.m_pPlayer.pev.punchangle[1] * 100, self.m_pPlayer.pev.punchangle[2] * 100, FALSE, FALSE);
...
function CGALIL:Reload() -- override
...
function CGALIL:WeaponIdle() -- override
...
function CGALIL:UseDecrement()  -- override {
    if CLIENT_WEAPONS then
		return TRUE
    else
        return FALSE
...
function CGALIL:GetKnockBackData() -- override
    return WeaponConfig.KnockBack
end

function CGALIL:GetMaxSpeed() -- override
    return WeaponConfig.PlayerMaxSpeed
end

function CGALIL:GetCSModelName()
    return WeaponConfig.W_Model
end

function CGALIL:iItemSlot()
    return WeaponConfig.ItemSlot
end

function CGALIL:GetBuyAmmoConfig()
